...
const generateRocDidDefaults = () => {
    ...
    rocDid.name = names[Math.floor(Math.random() * names.length)]
    ...
    const year = Math.floor(Math.random() * 20) + 70
...
const generateMoeNdcDefaults = () => {
    ...
    const studentIdPrefix = ['B', 'M', 'D'][Math.floor(Math.random() * 3)]
    const studentIdNumber = Math.floor(Math.random() * 100000000).toString().padStart(8, '0')
    ...
    moeNdc.major = majors[Math.floor(Math.random() * majors.length)]

    const year = Math.floor(Math.random() * 6) + 2019
    const month = Math.random() > 0.5 ? '06' : '01'
    const day = Math.random() > 0.5 ? '15' : '20'
    moeNdc.graduation_date = `${year}-${month}-${day}`

    moeNdc.institution = universities[Math.floor(Math.random() * universities.length)]
}
...
const generateToeicCertDefaults = () => {
    ...
    const candidateIdPrefix = ['A', 'B', 'C'][Math.floor(Math.random() * 3)]
    const candidateIdNumber = Math.floor(Math.random() * 100000000).toString().padStart(8, '0')
    ...
    const listeningScore = Math.floor(Math.random() * 251) + 200
    toeicCert.score_listening = listeningScore.toString().padStart(3, '0')

    const readingScore = Math.floor(Math.random() * 251) + 200
...
const generateMedicalCertDefaults = () => {
    ...
    // 隨機產生 3-7 天的休養天數
    const restDays = Math.floor(Math.random() * 5) + 3 // 3-7 天
    ...
    medicalCert.hospital_name = hospitals[Math.floor(Math.random() * hospitals.length)]

    // 醫師姓名
    const doctorLastNames = ['王', '李', '張', '陳', '林', '黃', '吳', '劉', '蔡', '楊']
    const doctorFirstNames = ['志明', '建國', '雅婷', '怡君', '俊傑', '淑芬', '家豪', '美玲', '承翰', '詩涵']
    medicalCert.doctor_name = doctorLastNames[Math.floor(Math.random() * doctorLastNames.length)] +
                               doctorFirstNames[Math.floor(Math.random() * doctorFirstNames.length)]

    // 醫師證號
    const licenseNumber = Math.floor(Math.random() * 100000).toString().padStart(6, '0')
